...
export const CallFeedback: FC<FeedbackSectionProps> = ({ id, onSubmitComplete }) => {
  ...
  const getFeedbackSection = (rating: number): JSX.Element | null => {
    ...
    if (rating === 5) {
      return (
        <>
          <span className="text-[#000000] text-base text-center">Any more suggestions?</span>
          <TextField
            value={comment}
            ...
            {issueOptions.map(({ text, value }) => {
              ...
              return (
                <div
                  ...
                  className={`text-base rounded-full border  py-1 px-2 cursor-pointer ${selected ? "border-[#5F99FC] text-primary-500" : "border-[#D8D8D8] text-[#47464F]"}`}
                >
                  {text}
                </div>
              );
  ...
  return (
    <>
      ...
      <span className="text-[#6B7280] text-base text-center">
        Please rate the quality, let us know what worked well, and share areas for improvement.
      </span>
      <span className={`text-[#000000] text-base ${rating === null ? "hidden" : ""}`}>
