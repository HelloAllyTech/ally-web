...
export const ReviewDetails = () => {
  ...
  const [showCommentsSidepanel, setShowCommentsSidepanel] = useState(true);
    ...
    <div className="h-full overflow-y-hidden">
      <div className="flex px-6 items-center gap-4 py-5 border-b-[0.5px]">
        ...
        <div className="flex flex-col justify-center gap-1.5 font-primary">
          <div className="text-xl font-medium text-typography-900">{HEADING.title}</div>
          <div className="flex gap-2 items-center text-gray-500">
            <div className="w-4 h-4 bg-neutral-500 rounded-full" />
            <div>By {HEADING.user.name}</div>
            <div className="w-1 h-1 bg-neutral-500 rounded-full mx-1" />
            <div>Date & time: {getFormattedDateTime(HEADING.date, "MMM dd, yyyy hh:mm a")}</div>
            <div className="w-1 h-1 bg-neutral-500 rounded-full mx-1" />
            <div>Duration: {getFormattedTimeFromDuration(HEADING.duration, "mm:ss")} Min</div>
          </div>
        </div>
      </div>
