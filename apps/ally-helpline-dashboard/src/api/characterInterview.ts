 ...
 * useCharacterInterviewStream, not an RTK endpoint. Ported from
 * ally-admin-dashboard's `src/api/characterInterview.ts`.
 *
...
const characterInterviewAPI = baseAPI.injectEndpoints({
  endpoints: builder => ({
    createCharacterInterviewSession: builder.mutation<CharacterInterviewSession, void>({
      query: () => ({
        url: ApiEndpoints.CHARACTER_LIBRARY.INTERVIEW_SESSIONS,
    ...
    getCharacterInterviewSession: builder.query<CharacterInterviewSession, string>({
      query: sessionId => ({
        url: ApiEndpoints.CHARACTER_LIBRARY.INTERVIEW_SESSION(sessionId),
