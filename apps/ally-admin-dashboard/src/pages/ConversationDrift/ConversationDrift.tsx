...
import { formatVersionLabel } from "@types";

import { AnalyticsTabFilters } from "../Analytics/analyticsFilters";
import { ChartCard, PALETTE, hBarOpts, lineOpts } from "../Analytics/chartKit";
import { sequentialScale, stableScale } from "../Analytics/chartScales";
...
/**
 * Kinds of drift: ONE colour, because the chart measures one thing (sessions) and
 * the category is already on the axis. It previously used ten distinct hues with
 * the legend off, so colour encoded nothing the label did not already say — and
 * four of them were raw inline hexes with no name anyone could look up.
 */
const kindsOpts = hBarOpts({
  bottomTitle: "Sessions affected",
  colorScale: {},
});
...
/**
 * Root cause, grouped by family so the two camps read without the caption having
 * to explain the colour code in prose: LLM causes cool, STT causes warm.
 */
const ROOT_CAUSE_COLOR: Record<string, string> = {
  "LLM (direct)": PALETTE.blue,
  "LLM: context lock-in": PALETTE.indigo,
  "STT (direct)": PALETTE.orange,
  "STT (cascade)": PALETTE.darkRed,
};
const rootCauseOpts = hBarOpts({
  bottomTitle: "Sessions",
  colorScale: ROOT_CAUSE_COLOR,
  legend: true,
});
...
/** Garble severity is an ORDERED pair, so saturation maps to severity. */
const GARBLE_SEVERITY_SCALE = sequentialScale(["Garble: partial", "Garble: severe"]);

const garbleOpts = hBarOpts({
  bottomTitle: "Sessions",
  colorScale: GARBLE_SEVERITY_SCALE,
});

/** STT error types are unordered, so they get stable per-name colours. */
const sttErrorOpts = hBarOpts({
  bottomTitle: "Sessions",
  colorScale: stableScale(Object.values(STT_INPUT_LABEL)),
});
...
export const ConversationDrift = ({ query, language }: AnalyticsTabFilters) => {
  ...
  const expBars = ratePctBars(expRows);
  ...
  return (
    // A tab body, not a page: no own <Theme>/<Section> and no second page
    // heading — the Analytics page owns both. `analytics-carbon` scopes chart CSS.
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="w-56">
              <Dropdown
                ...
                onChange={({ selectedItem }) => handleScenarioChange(selectedItem?.id ?? undefined)}
              />
            </div>
            {scenarioId != null && (
              <div className="w-44">
        ...
        {isError ? (
          ...
          <Tile className="p-6">
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              >
                <SimpleBarChart
                  data={driftRateBars}
                  options={hBarOpts({
                    bottomTitle: "Drift rate %",
                    colorScale: stableScale(driftRateBars.map(b => b.group)),
                  })}
                />
              </ChartCard>
            </div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              >
                <SimpleBarChart data={garbleSeverityBars} options={garbleOpts} />
              </ChartCard>
              <ChartCard
                bare
              ...
              >
                <SimpleBarChart data={errorTypeBars} options={sttErrorOpts} />
              </ChartCard>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              >
                <SimpleBarChart
                  ...
                  options={hBarOpts({
                    bottomTitle: "Drift rate %",
                    colorScale: stableScale(expBars.map(b => b.group)),
                  })}
                />
              </ChartCard>
            </div>
