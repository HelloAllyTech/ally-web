...
const budget = (used: number): RoadmapVoteBudget => ({
  periodKey: "2026-07",
  votesPerMonth: 100,
  used,
...
describe("maxFor", () => {
  it("adds back the votes already on this row", () => {
...
describe("clampVotes", () => {
  it("caps at the row ceiling rather than the raw budget", () => {
    expect(clampVotes(500, budget(100), 30)).toBe(30);
    expect(clampVotes(500, budget(60), 0)).toBe(40);
  });

  it("floors fractional input", () => {
    expect(clampVotes(7.9, budget(0), 0)).toBe(7);
  });

  it("treats junk and negatives as zero", () => {
    for (const raw of ["", "abc", NaN, undefined, null, -5, "-12"]) {
      expect(clampVotes(raw, budget(0), 0)).toBe(0);
    }
  });

  it("accepts numeric strings, which is what a text input gives us", () => {
    expect(clampVotes("42", budget(0), 0)).toBe(42);
  });
});
...
describe("remainingWithPending", () => {
  it("accounts for a local edit the server has not seen yet", () => {
    ...
    // tapped it up to 25 — so only 25 of the 40 are really left.
  ...
  it("is zero at the cap, which is what disables the vote button", () => {
