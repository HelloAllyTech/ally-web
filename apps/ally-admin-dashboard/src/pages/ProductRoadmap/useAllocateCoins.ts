 ...
 *
 * Generalised rather than copied when month boards arrived. The coin path has three non-obvious
 ...
 *    not exist and no-ops silently: the coins simply appear not to move, with no error anywhere.
 ...
 */
export const useAllocateCoins = (target: AllocationTarget) => {
   ...
   * entirely — and per invariant 1 a miss is SILENT, so this is the difference between coins
   * working and coins mysteriously not moving on one of the two layouts.
   */
    ...
    (opportunityId: string, mutate: (row: RoadmapOpportunity) => void) => {
      ...
      return store.dispatch(
        productRoadmapAPI.util.updateQueryData("getRoadmapBoard", target.args, draft => {
          for (const lane of [...draft.months, draft.unscheduled]) {
