...
/**
 * Grouped-bar series: Actual XP for every period, Goal only where a goal row
 * exists. Actual and Goal are the same unit and could in principle cross —
 * exactly the two-tiles-or-one-axis case `comboOpts` is NOT for — so both
 * ride one value axis as ordinary grouped bars.
 *
 ...
 * reads as a fact rather than a rendering glitch.
 */
export const buildGoalsXpSeries = (points: GoalsXpPoint[]): GoalsXpSeriesDatum[] => [
  ...points.map(p => ({ group: ACTUAL_GROUP, key: p.periodLabel, value: p.actualXp })),
  ...points
 ...
 * counting it against its goal now would understate it — and any period with
 * no goal, which has nothing to be met. Returns null when nothing qualifies.
 */
export const goalsXpTakeaway = (points: GoalsXpPoint[]): string | null => {
  const withGoal = points.filter(p => p.hasGoal && !p.inProgress && p.goalXp !== null);
...
/** True only when there is no XP to plot at all — a data gap, not a missing-goal fact. */
export const goalsXpEmptyText = (points: GoalsXpPoint[]): string | undefined =>
  points.length === 0 || points.every(p => p.actualXp === 0)
