 ...
 * (consistent title/caption + loading/error/empty handling) and the option
 * factories below so every chart shares the same axes/legend/toolbar defaults
 * and palette. Add new chart types here rather than hand-rolling option objects.
 */
...
/** Heatmap (x = `bottomMapsTo`, y = `leftMapsTo`, value = `value`). */
export const heatmapOpts = ({
  bottomMapsTo,
  leftMapsTo,
  bottomTitle = "",
  leftTitle = "",
  ...
  height,
  axes: {
    bottom: { title: bottomTitle, mapsTo: bottomMapsTo, scaleType: ScaleTypes.LABELS },
    left: { title: leftTitle, mapsTo: leftMapsTo, scaleType: ScaleTypes.LABELS },
  },
  heatmap: { colorLegend: { title: legendTitle } },
  toolbar: { enabled: false },
});
