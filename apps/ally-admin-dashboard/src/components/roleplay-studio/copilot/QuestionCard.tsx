...
/** Legacy `choice` is single-select; string options upgrade to {id,label}. */
const normalizeOptions = (options: CopilotQuestionEvent["options"]): CopilotQuestionOption[] =>
  (options ?? []).map(option =>
    typeof option === "string" ? { id: option, label: option } : option,
  );

/**
 * Structured `question` SSE events render as an answer card. Single-select and
 * free-text answer inline; multi-select and dropdown collect a set then Confirm.
 * "None of these" and an "add your own" entry are offered when the question
 * allows them. Once answered the card locks (also on resume).
 */
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  const summarizeAnswer = (answer?: CopilotStructuredAnswer): string | null => {
  ...
  const submitSingle = (option: CopilotQuestionOption) => {
  ...
  const renderBody = () => {
    ...
    if (question.kind === "singleSelect" || question.kind === "choice") {
      return (
        <>
    ...
    // Dropdown (long lists, e.g. languages).
    if (question.kind === "dropdown") {
      const selectedItems = options.filter(o => selected.includes(o.id));
      return (
        <div className="mt-3 flex flex-col gap-2">
          <FilterableMultiSelect
            id={`copilot-dropdown-${question.id}`}
            titleText=""
            placeholder={strings.selectPlaceholder}
            items={options}
            itemToString={(item: CopilotQuestionOption | null) => item?.label ?? ""}
            selectedItems={selectedItems}
            disabled={disabled}
            onChange={({
              selectedItems: next,
            }: {
              selectedItems: CopilotQuestionOption[] | null;
            }) => {
              setNoneSelected(false);
    ...
    return (
      <div className="mt-3 flex flex-col gap-1.5">
        {options.map(option => (
          <Checkbox
            key={option.id}
            id={`copilot-opt-${question.id}-${option.id}`}
            labelText={
        ...
        {customValues.map(value => (
          <Checkbox
            key={`custom-${value}`}
            id={`copilot-custom-opt-${question.id}-${value}`}
            labelText={value}
            checked
            disabled={disabled}
            onChange={(_event: unknown, { checked }: { checked: boolean }) => {
