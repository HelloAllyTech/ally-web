...
export const TooltipSidePanel: React.FC<TooltipSidePanelProps> = ({
  ...
  useEffect(() => {
    if (selectedTooltip) {
      ...
      setFormData({ location: "", tipText: "", icon: "", active: false });
    }
  }, [selectedTooltip]);
  ...
  const isFormValid = useMemo(() => {
    return !!(
      ...
      formData.tipText?.trim() &&
      formData.tipText.length <= 200
    );
  ...
  const handleClose = useCallback(() => {
    const original = selectedTooltip
      ...
      : { location: "", tipText: "", icon: "", active: false };
        ...
        <div className="h-[calc(100vh-100px)] px-10 pl-[46px] pt-2 overflow-y-auto custom-scrollbar">
          <div className="mb-4">
            ...
            <p className="text-sm text-typography-400 mt-1">
              The UI element or page section where the tooltip will appear (e.g., &quot;Login Button&quot;, &quot;Profile Icon&quot;)
            </p>
          </div>
