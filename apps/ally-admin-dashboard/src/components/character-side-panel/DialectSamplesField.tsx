  ...
  return (
    <div className="flex flex-col gap-2 w-full">
      {samples.map((sample, index) => (
        <div key={index} className="flex items-center gap-2">
          <input
            ...
            placeholder={en.simulation.dialectSamplePlaceholder}
      ...
      {samples.length < maxCount ? (
        ...
        >
          + {en.simulation.addDialectSample}
        </button>
      ) : (
        <span className="text-destructive-500 text-xs">* {en.simulation.dialectSampleLimit}</span>
      )}
    </div>
  );
};
