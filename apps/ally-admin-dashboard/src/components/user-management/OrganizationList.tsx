...
export const OrganizationList: React.FC<OrganizationListProps> = ({
  ...
  const renderHeader = (
    <div className="grid grid-cols-12 px-4 py-2 text-base text-text-500 border-b border-border-light">
      ...
      <div className="min-w-[900px]">
        ...
        <div className="h-[calc(100vh-270px)] overflow-y-auto">
          {organizations.map(tenant => (
            <div
              key={tenant.id}
              className="grid grid-cols-12 items-center px-4 py-3 text-text-700 border-b border-border-light hover:bg-background-secondary"
            >
              <div className="col-span-3 text-text-700">{tenant.name}</div>
              <div className="col-span-2 text-text-600">{tenant.code}</div>
              <div className="col-span-3 text-text-600 overflow-hidden whitespace-nowrap truncate w-[200px]">
                {tenant.description}
              </div>
              <div className="col-span-2 text-text-600">{formatDate(tenant.createdAt)}</div>
              <div className="col-span-2 flex items-center justify-between text-text-600 pl-1">
                <span>{tenant.userCount}</span>
                <button
                  className="text-text-400 hover:text-text-600 px-2"
