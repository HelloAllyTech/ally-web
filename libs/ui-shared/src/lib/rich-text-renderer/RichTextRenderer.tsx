 ...
 */
export const RichTextRenderer: FC<RichTextRendererProps> = ({ content, className = "" }) => {
  const sanitizedHtml = useMemo(() => {
    if (!content || content.trim().length === 0) return "";

    const normalizedContent = containsHtmlTags(content) ? content : `<p>${content}</p>`;

    return DOMPurify.sanitize(normalizedContent, PURIFY_CONFIG) as unknown as string;
  }, [content]);
  ...
  return (
    <div
      className={`rich-text-content prose prose-sm max-w-none text-typography-800 leading-relaxed [&_h1]:text-xl [&_h1]:font-bold [&_h1]:my-2 [&_h2]:text-lg [&_h2]:font-semibold [&_h2]:my-1.5 [&_h3]:text-base [&_h3]:font-semibold [&_h3]:my-1 [&_p]:my-1 [&_ul]:list-disc [&_ul]:pl-6 [&_ul]:my-1 [&_ol]:list-decimal [&_ol]:pl-6 [&_ol]:my-1 [&_li]:my-0.5 [&_blockquote]:border-l-4 [&_blockquote]:border-gray-300 [&_blockquote]:pl-4 [&_blockquote]:my-2 [&_blockquote]:text-typography-600 [&_blockquote]:italic [&_hr]:my-3 [&_hr]:border-gray-200 ${className}`}
