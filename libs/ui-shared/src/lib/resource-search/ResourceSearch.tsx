...
const ResourceSearch: FC<ResourceSearchProps> = ({
  ...
  const renderNoResults = () => {
    return <div className="text-left px-4 pt-[10px]">
  ...
  const renderResultsBody = (): React.ReactNode | null => {
    if(!searchQuery) {
      return <SuggestionsContainer isRow={isSuggestionsRow} suggestions={sampleSuggestions} onSelect={handleSearch} />;
    }
        ...
        <>
          ...
          <div style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }} className={`w-full pt-[14px] h-[90vh] overflow-y-auto flex flex-col gap-2 md:gap-4 items-center px-4 md:px-0 pb-[300px]`}>
            {filteredResources && filteredResources.length > 0 ? (
              <InfiniteScroll onInfiniteScroll={onInfiniteScroll || (() => {})} isLoading={isLoading || false}>
